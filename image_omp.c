...
void convolute(Image* srcImage,Image* destImage,Matrix algorithm){
    ...
    // Parallelize the outer row loop - each row can be processed independently
    // Each thread processes different rows, writing to different memory locations in destImage
    // Reading from srcImage is safe (read-only), and each thread writes to distinct destImage locations
